...
| 16 | **Comprehension gate** (field-guide #8): at verify/archive, generate an explainer ("what changed, why, with intuition") + a short quiz the user must pass — or explicitly waive — before archiving. Verifies the *owner* against the change, not just code against spec: the human's mental model is load-bearing for early regret detection, and autonomous waves erode it silently. Comprehension time is logged to the ledger as steering worth paying. | session 2026-07-07, Fable field guide | openspec `verify-change` + `archive-change` skill text (+ ledger convention) | S | Archiving offers explainer+quiz; pass/waive is recorded; a waive is visible in the ledger line. |
| 17 | **Reactable-options discipline** (field-guide #2): design/explore interviews present taste-shaped or wide-open forks as N concrete, contrasting options to *react to* (previews/artifacts where possible) instead of pure Q&A. Litmus in skill text: if the user must generate an answer rather than choose, and the fork is preference-shaped, produce options. Reacting is 3–5× cheaper steering than generating. | session 2026-07-07 | openspec `design` + `explore` skill text | S | The litmus + instruction appear in both skills; the next taste-fork is presented as reactable options. |
| 18 | **Blind-spot pass step** (field-guide #1): explore (and the #11 inception diagnostic) opens unfamiliar territory by *inverting* the interview — "what does the user not know they don't know about this domain that would change the WHAT" — before any requirement question. Triggered by declared unfamiliarity or detected novelty. | session 2026-07-07 | openspec `explore` skill text; folded into #11 as its opening move | S | Explore's flow contains the inversion step + trigger rule; #11's diagnostic opens with it. |
| 19 | **Skill-granularity counterfactual** (ledger experiment): run one comparable wave twice — full ritual skill text vs. minimal prompt, same model class — and compare cost/quality via ledger + review. Step-by-step imperatives are themselves compensators for drift-prone models and may *cap* high-reasoning ones; the verdict decides whether instruction granularity gets a retirement condition in the #3 compensator rule. | session 2026-07-07, blind-spot pass | ledger experiment + #3 rule extension | S | Both runs ledgered and compared; verdict recorded in ops; #3's rule extended (or explicitly not) with rationale. |
| 20 | **WHAT-freshness pass** (retro variant): periodic cold re-read of a rotating spec slice asking "is this still what you'd choose today?" — detects correct-but-no-longer-wanted specs (the GH#1 drift pattern, generalized). Forward verification (code⇄spec) and failure-triggered retro both miss preference drift. | session 2026-07-07 | retro skill (#2) checklist extension | S | The per-milestone retro includes a rotating spec-freshness slice; first pass runs on the post-GH#1 baseline. |
