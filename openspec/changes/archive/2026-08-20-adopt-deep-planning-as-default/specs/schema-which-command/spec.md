...
- **WHEN** user runs `openspec schema which deep-planning` and no override exists
...
- **WHEN** user runs `openspec schema which deep-planning` and both project and package have `deep-planning`
...
- **WHEN** user runs `openspec schema which deep-planning --json`
- **THEN** system outputs JSON with `name`, `source`, and `path` fields

#### Scenario: JSON output with shadows

- **WHEN** user runs `openspec schema which deep-planning --json` and schema has shadows
