...
Archiving folds a change's deltas into your main `openspec/specs/` and moves the change folder to `openspec/changes/archive/YYYY-MM-DD-<name>/` (an already-dated change name is kept as is). Re-archiving deltas that were already synced is a warned no-op, not a failure, so a re-run cannot corrupt `specs/`. Because `specs/` is the **shared source of truth**, the timing matters on a team. Two workable conventions:
