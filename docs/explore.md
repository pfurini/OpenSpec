...
**`/openspec-explore` is your thinking partner. Reach for it whenever you have a problem but not yet a plan.** It investigates your codebase, weighs options with you, and clarifies what you actually want, all before a single artifact or line of code is created. When the picture is clear, it hands off to `/openspec-propose`.
...
Skip explore only when you already know exactly what you want and how. In that case go straight to [`/openspec-propose`](commands.md#opsxpropose).
...
- Transition to `/openspec-propose` when you're ready.
...
Good news: `/openspec-explore` ships in the default **core** profile, right alongside `propose`, `apply`, `sync`, and `archive`. You don't need to enable anything. If OpenSpec is set up in your project, explore is ready in your AI chat. (As with all `/openspec-*` commands, you type it in your assistant's chat, not the terminal. See [How Commands Work](how-commands-work.md).)
...
You: /openspec-propose add-order-idempotency-key
...
You can say it in plain language ("let's turn this into a change") or run `/openspec-propose <name>` directly. Either way, the exploration you just did becomes the foundation of the proposal, not throwaway chat.

If you use the expanded command set, explore can hand off to `/openspec-new-change` instead, for step-by-step artifact creation. See [Workflows](workflows.md).
...
- **Explore again mid-change.** Stuck during `/openspec-apply-change`? You can step back and explore a sub-problem, then return.
...
**What it costs:** a little patience. Explore is a conversation, so it's slower than firing off `/openspec-propose` and hoping. For work you genuinely understand already, that extra step is pure overhead, and you should skip it.
...
- [Commands: `/openspec-explore`](commands.md#opsxexplore): the precise reference
