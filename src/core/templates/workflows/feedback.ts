/**
 * Skill Template Workflow Module — feedback.
 *
 * Authored as a directory under `schemas/skills/feedback/`
 * (`SKILL.md` with frontmatter); this factory loads it.
 */
import type { SkillTemplate } from '../types.js';
import { loadSkillSource } from '../../shared/skill-bundle.js';

const source = loadSkillSource('feedback');

export function getFeedbackSkillTemplate(): SkillTemplate {
  const { frontmatter } = source;
  if (!frontmatter.name || !frontmatter.description) {
    throw new Error("feedback SKILL.md must define 'name' and 'description' in frontmatter");
  }
  return {
