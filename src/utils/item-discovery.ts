...
export async function getSpecIds(root: string = process.cwd()): Promise<string[]> {
  // One discovery contract everywhere: recursive walk, path ids, code-point
  // sort. A flat walk here would make nested specs invisible to validate,
  // show, the pickers, and completions while spec list displays them.
  const discovered = await discoverSpecFiles(path.join(root, 'openspec', 'specs'));
